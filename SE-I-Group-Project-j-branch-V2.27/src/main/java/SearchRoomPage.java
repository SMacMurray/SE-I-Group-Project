...
public class SearchRoomPage extends JFrame{
    ...
    private static boolean checkForExceptions(JTextField bdcf, JTextField gcf, JTextField brf) {
		if (bdcf.getText().isEmpty() || gcf.getText().isEmpty() || brf.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Can't have empty entries.");
			return true;
		}
		int beds = Integer.parseInt(bdcf.getText());
		int guestCount = Integer.parseInt(gcf.getText());
		double baseRate = Double.parseDouble(brf.getText());
    	if (beds <= 0) {
			JOptionPane.showMessageDialog(null, "Can't have 0 beds for a room.");
			return true;
		}
		else if (guestCount <= 0) {
			JOptionPane.showMessageDialog(null, "Can't have 0 max occupancy for a room.");
			return true;
		}
		else if (baseRate < 0) {
			JOptionPane.showMessageDialog(null, "Can't have a base rate of 0 for a room.");
			return true;
		}
    	return false;
    }
    SearchRoomPage() {
		...
		titleWrapperPane.setBorder(BorderFactory.createEmptyBorder(0, Main.WINDOW_H - 130, 0, 0));

		alignHomePane.add(titleWrapperPane);
		...
		searchRoomButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (checkForExceptions(bedCountField, guestCountField, baseRateField)) { // Not completed yet
					return;
				}
